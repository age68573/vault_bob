...
- Dashboard 可顯示 MongoDB 健康狀態、查詢客戶清單，並建立客戶資料。
...
| `SMTP_USERNAME` | `batch-notifier@example.internal` | 寄送系統通知信件使用的 SMTP 帳號。 | `secret/data/vault-migration-demo/smtp` |
| `SMTP_PASSWORD` | `demo-smtp-password` | SMTP 帳號密碼。 | `secret/data/vault-migration-demo/smtp` |
...
2. 遷移 JWT、webhook HMAC 與 keystore 密碼，避免簽章及 TLS 密鑰長期存在於原始碼。
3. 遷移外部 API、SMTP 與 legacy Basic Auth 認證資訊。
